...
const ArticleEditor = ({ articleId }: { articleId?: number }) => {
  ...
  const handleShare = async (permission: string, expiresIn: string) => {
    if (!articleId) return;

    try {
      const response = await axios.post(`http://localhost:5000/api/articles/${articleId}/share`, {
        permission_level: permission,
        expires_days: expiresIn,
      });

      return response.data.shareUrl; // Возвращаем созданную ссылку
    } catch (error) {
      toast({
        title: 'Ошибка',
        description: error?.response?.data?.message || 'Не удалось создать ссылку',
        variant: 'destructive',
      });
  ...
  // Вызовите handleShare в ArticleHeader
